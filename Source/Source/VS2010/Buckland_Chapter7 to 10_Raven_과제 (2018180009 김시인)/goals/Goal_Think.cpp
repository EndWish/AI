...
Goal_Think::Goal_Think(Raven_Bot* pBot):Goal_Composite<Raven_Bot>(pBot, goal_think)
{
  ...
  double AvoidBias = RandInRange(LowRangeOfBias, HighRangeOfBias);
  ...
  m_Evaluators.push_back(new AvoidGoal_Evaluator(AvoidBias)); // 큐에 평가자 삽입
...
// 목표 추가하기
void Goal_Think::AddGoal_Avoid() {
    if (notPresent(goal_avoid)) {
        RemoveAllSubgoals();
        AddSubgoal(new Goal_Avoid(m_pOwner));
    }
}
