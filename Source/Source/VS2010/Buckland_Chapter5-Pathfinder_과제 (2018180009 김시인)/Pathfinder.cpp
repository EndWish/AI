...
//
//------------------------------------------------------------------------
void Pathfinder::CreatePathIDS() {
    //set current algorithm
    m_CurrentAlgorithm = search_ids;
...
void Pathfinder::CreatePathAStar()
{
  ...
  typedef Graph_SearchAStar<NavGraph, Heuristic_Manhattan> AStarSearch;  //¸àÇÏÅº ÈÞ¸®½ºÆ½À» »ç¿ëÇÑ Search »ý¼º[¼öÁ¤]
...
std::string Pathfinder::GetNameOfCurrentSearchAlgorithm()const
{
  ...
  case search_ids: return "Iterative Deepening Depth First";
