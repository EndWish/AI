...
public:
    ...
    Graph_SearchIDS(const graph_type& graph,
        int          source,
        int          target = -1) :
...
template <class graph_type>
bool Graph_SearchIDS<graph_type>::Search() {
    std::vector<int> fence(m_Visited.size(), 99999);
    int depthMax = 0;
    ...
    while (true) {
        ...
        //create a std stack of edges
        std::stack<const Edge*> stack;  //depth를 같이 저장
        ...
        while (!stack.empty()) {
            ...
            m_Visited[Next->To()] = visited;
            if (Next != &Dummy) {
                fence[Next->To()] = fence[Next->From()] + 1;
            }
            ...
            for (const Edge* pE = ConstEdgeItr.begin(); !ConstEdgeItr.end();  pE = ConstEdgeItr.next()) {
                if (m_Visited[pE->To()] == unvisited) {
                    if(fence[pE->From()] < fence[pE->To()])
                        stack.emplace(pE);
                }
                else {
                    fence[pE->To()] = min(fence[pE->To()], fence[pE->From()] + 1);
                }
            }
        }

        ++depthMax;
        if((int)m_Visited.size() < depthMax)
            break;
...
template <class graph_type>
std::list<int> Graph_SearchIDS<graph_type>::GetPathToTarget()const {
    std::list<int> path;
